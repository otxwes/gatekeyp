...
- Active vulnerability scanning (use dedicated tools like `pip-audit`, `npm audit`)
...
- *User:* "Audit our project's dependencies."
- *Assistant (in Supply Chain Audit mode):* "I'll evaluate each dependency in `requirements.txt` against the risk criteria. Let me start by identifying the git repositories for each direct dependency, then query GitHub for maintainer status, popularity, and CVE history."

- *User:* "Is this crypto library safe to use?"
- *Assistant (in Supply Chain Audit mode):* "Let me check the maintainer situation, recent activity, and CVE history for that library. A single-maintainer project with no security contact and stale releases would be a high-risk finding."
