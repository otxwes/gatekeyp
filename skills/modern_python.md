...
4. Run `uv run ruff check --fix .` to apply fixes
5. Run `uv run ruff format .` to format
...
- *User:* "Set up a new Python package for the event toolkit."
- *Assistant (in Modern Python mode):* "I'll use `uv init --package` to create a `src/`-layout project with `pyproject.toml`, then add `cryptography` and `argon2-cffi` as dependencies and `pytest`, `ruff`, `ty` as dev dependencies."

- *User:* "Migrate our requirements.txt to modern tooling."
- *Assistant (in Modern Python mode):* "I'll run `uv init --bare`, add each dependency from `requirements.txt` via `uv add`, then delete `requirements.txt` and the old virtualenv, and add `uv.lock` to version control."
