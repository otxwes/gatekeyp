...
- Test suite: 290 tests passing (`uv run pytest`; the venv's `cryptography` wheel is x86_64 on Apple Silicon — `arch -x86_64 python -m pytest -q` also works). Plus `python -m tests.jxa_stego_check` for the JS↔Python codec agreement (now incl. `coverFit` pins + `keyLine`/`parseKeyLine`), and `python -m tests.stego_ref --write-fixture` to regenerate the invite-card fixture.
