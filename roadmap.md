...
## Phase B: RSVP Pull-Flow Funnel (Organizer Approval Gate)
*Goal: attendees request a seat instead of pasting a key; the organizer approves, and only then does the attendee receive a pre-minted one-time access key — delivered as a single-show secret. Invite artifacts become opaque and stego-only: the printed QR is removed, because a scannable key is a secrecy downgrade (anything photographable can be harvested en masse without the harvester ever suspecting steganography).*

- [x] Web — attendee RSVP section on the door (`web/app.js`): "Request a seat" form with a `website` honeypot field (a bot fill gets a canned acknowledgement; nothing is stored or minted), passphrase gate when the organizer sets one, pending/approved result states, and the single-show access key revealed once on approval. — **Done 2026-09-13 (web half, commit `ccc0b8c`).**
- [x] Web — organizer workspace RSVP tab (`web/app.js`): copyable share link, gate settings (require passphrase + auto-approve toggle, passphrase re-entry rule), and the request queue with per-row approve/deny. — **Done 2026-09-13 (web half, commit `ccc0b8c`).**
- [x] Web — opaque, stego-only invite cards: `qrPayload`/`parseQrPayload` renamed to `keyLine`/`parseKeyLine` in `web/stego.js` (same `gkp:` text format — the paste-anywhere escape hatch); the printed-QR fallback is removed from the card renderer; mirrored in `tests/stego_ref.py` + the JXA harness. — **Done 2026-09-13.**
- [x] Web — QR removal: `web/door_qr.js`, `web/vendor/jsqr.js`, `web/vendor/qrcode-generator.js` (+ LICENSEs) deleted; `web/vendor/` gone; no QR references remain in `web/`. — **Done 2026-09-13.**
- [x] Tests — JXA harness/oracle follow the rename; the static-serving test pins the removed door assets to 404; suite 290 passing, ruff clean. — **Done 2026-09-13.**
- [x] Server — RSVP routes/service (`src/rsvp/`), gateway wiring, database tables for the request queue and pre-minted approved-but-unclaimed keys: submissions pre-mint an access key stored **only as a keyed HMAC** and returned exactly once; zero content grants until organizer approval (or the auto-approve dial); denial revokes; per-IP submission limiter (5 per 600 s, `Retry-After`), pending-queue cap (500/event), `website` honeypot; `wipe_event` removes RSVP rows and their pre-minted keys in one transaction. — **Done 2026-09-13 (commit `ec1a141`).**
- [x] Threat-model review of server-held pre-minted keys — four flagged questions cleared (at-rest HMAC-only storage; 30-day door-enforced TTL; three-layer request-queue bounding; auto-approve dial bounded + semantics corrected to "up to N currently-approved", now tested). — **Done 2026-09-13.**
- [x] Docs — threat-model note for the pull-flow (server holds pre-minted approved-but-unclaimed keys) written into `docs/threat_model.md` (§ Phase B Status); suite now 293 passing, ruff clean. — **Done 2026-09-13.**
- [x] Live E2E QA — full funnel driven against the running server on :8000 (clean arm64 QA venv at `/tmp/gkpqa`): create event → content block → passphrase gate (wrong passphrase → 400; correct → 201 pending + single-show key returned once) → pending key at the door gets "awaiting approval" with no content → organizer approve → door unlocks the payload → organizer list shows decrypted contact + hash-only keys; auto-approve dial N approves while approved_count < N (cap verified live); 6th per-IP submission → 429 `Retry-After: 600`; honeypot `website` fill → canned ack, no row minted, bypasses the limiter even at a full window; deny → door rejects the key; re-submission after denial → fresh pending row; decommission → wipe counts in the response, public view `{"status":"ended"}`, late submission → 410 Gone, door/list reject revoked keys. QA caught a real gap: the decommission route only revoked keys and never wiped/tombstoned (post-decommission submissions still minted keys; the view stayed "live") — fixed in `a9a6fc0` with a lifecycle-level regression test; suite 294 passing, ruff clean. — **Done 2026-09-14.**
...
- Current focus: Phase 3 - Frontend & UX/UI Design is **complete** (full SPA in `web/app.js`; redesign in `web/style.css`; markup repaired in `web/index.html`). Phase 3.5 - UX & Visual Design Iteration is **complete** (Monochrome Edition: B&W palette + motif set §19, a11y + QA matrix). Phase 3.6 - Steganographic Invite Keys is **complete** (codec + card + QR encode/fallback + organizer/attendee UX + door QR decode + honest rejection + share guidance + docs + tests). Phase 3.7 - Custom Card Covers is **complete** (preset monochrome covers + own-image upload drawn cover-fit; client-side only). Phase A - Ephemeral ("Lite") Events (embeddable pivot, first step) is **complete** (`src/ephemeral/`, `#/flyer` funnel, `#/e/{id}` attendee door, self-wipe + tombstones, `GATEKEYP_PROFILE=lite` server profile; live QA on 2026-09-08 passed end-to-end). Phase B - RSVP Pull-Flow Funnel is **complete** (web half `ccc0b8c`; server half `ec1a141`: `src/rsvp/` routes/service, gateway pending gate, pre-minted HMAC-only keys with zero grants until approval, request-queue rate limiting + 500/event pending cap, DB migrations + wipe cascade; threat-model review passed 2026-09-13; live E2E QA passed 2026-09-14 and caught + fixed the decommission wipe/tombstone gap (`a9a6fc0`); suite 294 passing, ruff clean).
- Next: Terminology rename "master" -> "organizer" landed as a hard cutoff (`9513c1a`): legacy `master_key` field/param/kwarg, `GATEKEYP_MASTER_KEY` env var, and the `MissingMasterKeyError` alias are all removed and now fail loudly; `tests/test_organizer_key_compat.py` pins the rejection (6 tests). Then: Phase A+ embeddable surface (iframe/script widget, richer OG cards — proposed, needs scoping + threat-model boundary) and/or Phase 4 - Map & Navigation. Invite-card art reuses the Phase-3.5 motif primitives (`--hatch-*`, `.voided`) — design-system §12.
- Test suite: 300 tests passing (`uv run pytest` — venv is native arm64;
  `cryptography`'s `_rust.abi3.so` verified arm64 via `file`, so the old
  `arch -x86_64 python -m pytest` workaround is obsolete). Plus `python -m tests.jxa_stego_check` for the JS↔Python codec agreement (now incl. `coverFit` pins + `keyLine`/`parseKeyLine`), and `python -m tests.stego_ref --write-fixture` to regenerate the invite-card fixture.
